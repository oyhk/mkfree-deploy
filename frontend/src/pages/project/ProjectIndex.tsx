...
const columns: ProColumns<ProjectDto>[] = [
  {
    ...
    dataIndex: 'name',
    key: 'name',
    render: (_, row) => <Link to={`/project/edit/${row.id}`}>{row.name}</Link>,
  },
  {
    title: '操作',
    dataIndex: 'action',
      ...
      render: (projectEnvServerList: ProjectEnvServerDto[]) => (
        <div>
          {projectEnvServerList
            ? projectEnvServerList.map((pes) => (
                <span className={styles.ipRow} key={`${pes.id}_${pes.envId}_${pes.serverIp}_span`}>
                  {pes.serverIp}
                  <br key={`${pes.id}_${pes.envId}_${pes.serverIp}_br`} />
                </span>
              ))
            : ''}
        </div>
    ...
    },
    {
      ...
      render: (projectEnvServerList: ProjectEnvServerDto[]) => (
        <div>
          {projectEnvServerList
            ? projectEnvServerList.map((pes) => (
                <span className={styles.ipRow} key={`${pes.id}_${pes.envId}_${pes.publishTime}`}>
                  {pes.publishTime}
                  <br />
                </span>
              ))
    ...
    },
    {
      ...
      render: (projectEnvServerList: ProjectEnvServerDto[]) => (
        <div>
          {projectEnvServerList
            ? projectEnvServerList.map((pes) => (
                <span className={styles.ipRow} key={`${pes.id}_${pes.envId}_${pes.publishVersion}`}>
                  {pes.publishVersion}
                  <br />
                </span>
              ))
          ...
          {projectEnvServerList
            ? projectEnvServerList.map((pes) => (
                <div
                  ...
                  key={`${pes.projectId}_${pes.id}_${pes.envId}_${pes.publishVersion}`}
                >
                  {pes.isPublish ? (
    ...
    <PageHeaderWrapper>
      <ProTable<ProjectDto>
        ...
        toolBarRender={(action) => [
          <Link to="/project/create" type="primary">
            添加
          </Link>,
        ]}
      />
...
export default connect(({ project, dispatch }: ProjectPageProps) => ({ project }))(ProjectPage);
