...
import { Button, Form, Input, Select, Switch, Radio, Row, Col, Checkbox } from 'antd';
...
const ProjectForm: React.FC<ProjectEditPageProps> = ({ project, dispatch }) => {
  ...
  return (
    <Form
      ...
      onFinish={(projectDto) => {
        console.log('project form submit', projectDto);
    ...
    >
      ...
      <Form.List name="projectEnvList">
        {(fields, { add, remove }) => {
          ...
          // 已经选中的环境Id列表
          const selectedEnvIdList = fields.map(
            (projectEnvListField) => projectEnvListField?.record?.envId,
          );
          return (
            <div>
              <Row>
                  ...
                  <Form.Item noStyle>
                    <Select
                      ...
                      onSelect={(envId, option) => {
                        ...
                        const projectEnvServerList = project?.serverList?.map(
                          (server: ServerDto) => {
                            return {
                              envId: server.envId,
                  ...
                  >
                    {(fields) => {
                      ...
                      fields?.forEach((projectEnvServerField, projectEnvServerListIndex) => {
                        projectEnvServerField.record =
                          projectEnvListField?.record.projectEnvServerList[
                            projectEnvServerListIndex
                          ];
                      });
                      return (
                        <div>
                          {fields.map((projectEnvServiceField, projectEnvServerListIndex) => (
                            <Row style={{ lineHeight: '32px' }} key={uuid()}>
                              ...
                              <Col xl={8}>
                                ...
                                >
                                  <Checkbox>
                                    {projectEnvServiceField?.record?.serverName}-
                                    {projectEnvServiceField?.record?.serverIp}
                                  </Checkbox>
                                </Form.Item>
                              </Col>
                              <Col
                  ...
                  >
                    <Radio.Group buttonStyle="solid">
                      {project?.serverList?.map((server) => {
                        return (
                          <Radio.Button value={server.ip} key={uuid()}>
                    ...
                    {(fields, { add, remove }) => {
                      return (
                        <div>
                          ...
                          {fields.map(
                            (projectBuildBeforeListField, projectBuildBeforeListIndex) => (
                              <Row key={uuid()}>
                                <Col
                                  xl={4}
